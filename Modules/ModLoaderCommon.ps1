...
function Get-InstalledModLoaders {
    ...
    try {
        ...
        $versionModLoaders = $modLoaders | Where-Object { $_.MinecraftVersion -eq $MinecraftVersion }
        
        if ($versionModLoaders) {
...
function Start-ModLoaderGame {
    ...
    Write-Host "Launching Minecraft $MinecraftVersion with $ModLoaderType $ModLoaderVersion..." -ForegroundColor Green
